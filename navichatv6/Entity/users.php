...
class User
{
    ...
    public function createUser($name, $email, $password, $company, $phone, $industry)
    {
        $stmt = $this->conn->prepare(
            "INSERT INTO users (name, email, password, company, phone, industry, access) VALUES (?, ?, ?, ?, ?, ?, 0)"
        );
        $stmt->bind_param("ssssss", $name, $email, $password, $company, $phone, $industry);
        return $stmt->execute();
    }
